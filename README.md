...
> **基于 Spark 的电商用户行为分析与转化优化**
>
> - 在 Hadoop + Spark 分布式集群上使用 PySpark 处理 1 亿条淘宝用户行为日志，覆盖数据清洗、时间解析与特征工程
> - 构建 pv→cart→fav→buy 四层转化漏斗模型，定位用户流失关键环节；使用窗口函数计算商品复购率与品类渗透率
> - 分析 24 小时流量分布与周内购买模式，识别晚间 20-22 点为流量和转化双高峰
> - 基于长尾分析发现 Top 20% 商品贡献约 80% 销量，提出差异化运营策略
